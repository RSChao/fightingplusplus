...
public class peru {
    ...
    static Technique lilPoisonDarts = new Technique(
        "lil_poison_darts",
        "Lil' poison darts",
        new TechniqueMeta(false, 180000, List.of("Lil' poison darts")),
    ...
    static Technique annoyingGlitch = new Technique(
        "annoying_glitch",
        "Annoying glitch",
        new TechniqueMeta(false, 240000, List.of("Annoying glitch")),
        TargetSelectors.self(),
        (ctx, token) -> {
            Player player = ctx.caster();
    ...
    static Technique jumpGoBig = new Technique(
        "jump_go_big",
        "Jump go big",
        new TechniqueMeta(false, 360000, List.of("Jump go big")),
        TargetSelectors.self(),
        (ctx, token) -> {
            Player player = ctx.caster();
    ...
    static Technique dontBlowMeUp = new Technique(
        "dont_blow_me_up",
        "Dont blow me up",
        new TechniqueMeta(false, 300000, List.of("Dont blow me up")),
        TargetSelectors.self(),
        (ctx, token) -> {
            Player player = ctx.caster();
    ...
    static Technique ultimateGlitch = new Technique(
        "ultimate_glitch",
        "Ultimate glitch",
        new TechniqueMeta(true, cooldownHelper.secondsToMiliseconds(3600), List.of("Ultimate: Ultimate glitch")),
        TargetSelectors.self(),
        (ctx, token) -> {
            Player player = ctx.caster();
            for (ItemStack item : player.getInventory().getArmorContents()) {
