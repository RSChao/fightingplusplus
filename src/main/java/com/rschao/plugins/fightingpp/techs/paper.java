...
public class paper {
    ...
    static Technique paperSpawn = new Technique(
        "paper_spawn",
        "Paper Spawn",
        new TechniqueMeta(false, cooldownHelper.secondsToMiliseconds(60), List.of("Paper Spawn")),
    ...
    static Technique healWithPaper = new Technique(
        "heal_with_paper",
        "Heal with Paper",
        new TechniqueMeta(false, cooldownHelper.secondsToMiliseconds(180), List.of("Heal with Paper")),
        TargetSelectors.self(),
        (ctx, token) -> {
            Player player = ctx.caster();
            int paperCount = 0;
    ...
    static Technique paperArmor = new Technique(
        "paper_armor",
        "Paper Armor",
        new TechniqueMeta(false, cooldownHelper.secondsToMiliseconds(300), List.of("Paper Armor")),
        TargetSelectors.self(),
        (ctx, token) -> {
            Player player = ctx.caster();
            int paperCount = 0;
    ...
    static Technique paperBlade = new Technique(
        "paper_blade",
        "Paper Blade",
        new TechniqueMeta(false, cooldownHelper.secondsToMiliseconds(360), List.of("Paper Blade")),
        TargetSelectors.self(),
        (ctx, token) -> {
            Player player = ctx.caster();
            int paperCount = 0;
    ...
    static Technique mayhemOfPaper = new Technique(
        "mayhem_of_paper",
        "Mayhem of Paper",
        new TechniqueMeta(true, cooldownHelper.secondsToMiliseconds(3600), List.of("Ultimate: Mayhem of Paper")),
        TargetSelectors.self(),
        (ctx, token) -> {
            Player player = ctx.caster();
            int paperCount = 0;
